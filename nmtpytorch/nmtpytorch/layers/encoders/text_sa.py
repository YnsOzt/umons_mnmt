...
class SATextEncoder(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, n_vocab, rnn_type, ff_size=2048,
                 n_head=8, num_sa_layers=6, num_layers=1, bidirectional=True,
                 dropout_rnn=0, dropout_emb=0, dropout_ctx=0, dropout_sa=0.1,
        ...
        self.enc_list = nn.ModuleList(
            [SelfAttention(self.ctx_size, self.n_head, self.ff_size, self.dropout_sa) for _ in
             range(self.num_sa_layers)])
    ...
    def forward_same_len_batches(self, x):
        ...
        hs = hs.transpose(1, 0)  # (t, BS, hidden) => (BS, t, hidden)
        
        for sa in self.enc_list:
            hs = sa(hs, None)

        hs = hs.transpose(1, 0)  # (BS, t, hidden) => (t, BS, hidden)
