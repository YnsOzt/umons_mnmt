...
class TextEncoderCTXMUL(nn.Module):
    """A recurrent encoder with embedding layer. this class merges the visual fead ( in forward method ) with the textual one
    ...
    """
    def __init__(self, input_size, hidden_size, n_vocab, rnn_type, ctx_size_dict,
        ...
        self.pool_transform = FF(self.ctx_size_dict['image'], self.ctx_size, bias=False, activ="tanh")

    def forward(self, x, img_feats):
        """
        :param x: sentences (TLS x BS)
        :param img_feats: image features (n x BS x FeatSize)
        :return: the hidden state and a mask if necessary
        """
        if (x == 0).nonzero().size():
            return self.forward_mixed_len_batches(x, img_feats)
        else:
            return self.forward_same_len_batches(x, img_feats)

    def forward_same_len_batches(self, x, img_feats):
        # Fetch embeddings
        embs = self.emb(x)
        ...
        img_feats_tanh = self.pool_transform(img_feats[0]).squeeze(0)

        hs = img_feats_tanh * hs

        # No mask is returned as batch contains elements of all same lengths
        return hs, None

    def forward_mixed_len_batches(self, x, img_feats):
